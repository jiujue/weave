...
function makeRow(id: string): TableRow {
	...
	const name = `${pick(familyNames)}${pick(givenNames)}${Math.random() < 0.25 ? pick(givenNames) : ''}`
	return {
		id,
		cells: {
			name,
...
export function WeaveDemo() {
	...
	const message = useMemo(() => {
		return `rows=${rows.length} height=${tableHeight === 0 ? 'auto' : `${tableHeight}px`} seed=${seed}`
