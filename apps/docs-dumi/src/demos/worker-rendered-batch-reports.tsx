			...
			<Container id='metaRow' style={{ flexDirection: 'row', gap: 18 }}>
				...
				>
					...
					<Text id='metaLeftLine1' textStyle={body}>
						本页展示：WebWorker 渲染 A4 PNG（主线程只显示
						img），用于性能与大批量生成场景。
					</Text>
...
export default function Demo(): JSX.Element {
	...
	const rows = useMemo(
		() => rowsFor(variant, dataset.id),
		[dataset.id, variant]
	)
	const scene: SceneNode = useMemo(
		() =>
			sceneFromJSX(
				buildReportScene({ ...a4, variant, dataset, columns, rows })
			),
		...
		<div style={{ display: 'grid', gap: 10 }}>
			...
			<div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
				<div style={{ fontSize: 12, color: '#334155' }}>
					状态：{status}｜当前：{variantName(variant)} · #
					{String(index).padStart(3, '0')} · {dataset.customer}
				</div>
				<button
					type='button'
