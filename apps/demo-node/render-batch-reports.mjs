...
if (envFontPath && envFontFamily) {
	if (tryRegisterFont(envFontPath, envFontFamily))
		primaryFontFamily = envFontFamily
...
const rowsFor = (v, datasetId) => {
	const rnd = mulberry32(
		2000 + datasetId + (v === 'invoice' ? 0 : v === 'reimburse' ? 100 : 200)
	)
...
const variants =
	variant === 'all' ? ['invoice', 'reimburse', 'statement'] : [variant]
...
console.log(
	`Breakdown: render=${renderMs.toFixed(0)}ms, write=${writeMs.toFixed(0)}ms`
)
console.log(
	'Tip: use --variant=all to render 3 styles; use --id=1..100 to render one.'
)
