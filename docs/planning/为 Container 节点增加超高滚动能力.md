...
- core
  ...
  - 在 `buildDisplayList()` 的 container 分支加入 `clipRect`，并在递归 children 前 `translate(-scroll)` -（可选）新增 `hitTest(point)`：维护 transform+clip 栈，保证滚动后命中正确
- adapter（可选）
...
- 在 docs 示例中验证：
  - 内容超出被裁剪
  - scroll 更新后内容平移且 clamp 正确 -（若做 hitTest）滚动后点击命中仍正确
