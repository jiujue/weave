...
const initialScene = sceneFromJSX(
	...
	>
		...
		>
			<text id="logo" textStyle={{ fontSize: 20, fontWeight: 700, color: '#fff' }}>
				Weave Editor
			</text>
		...
		<container id="content" style={{ flexGrow: 1, flexDirection: 'row', padding: 20, gap: 20 }}>
			<container
				id="card1"
				style={{
					width: 200,
...
function isParentableNode(node: SceneNode): boolean {
	return node.type === 'container' || node.type === 'relative' || node.type === 'table'
}
...
export function moveNode(
	...
	if (position === 'inside') {
		...
		if (!newTargetParent || !('children' in newTargetParent) || !newTargetParent.children)
			return root

		const targetIndex = newTargetParent.children.findIndex((c) => c.id === targetId)
