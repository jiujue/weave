			...
			{def.props.map((prop) => {
				...
				return (
					<div key={prop.name} className="flex flex-col gap-1">
						<label className="text-xs font-medium text-slate-500">{prop.label}</label>
						{prop.type === 'string' && (
							<input
								...
								<textarea
									...
									onBlur={() => {
										...
										try {
											...
											setJsonErrors((prev) => ({
												...prev,
												[jsonKey]: err instanceof Error ? err.message : 'Invalid JSON',
											}))
										}
									}}
								/>
								{jsonError && <div className="text-xs text-red-600 font-mono">{jsonError}</div>}
							</>
						)}
					</div>
				)
